...
本项目提供完整的 IPTV 直播源解决方案，核心能力：

- **📺 咪咕视频源抓取** — 自动获取 CCTV、卫视等频道（720P 高清；配置咪咕 VIP 账号可解锁体育直播及 1080P / 蓝光更高画质）
- **🔗 外部源管理** — 外部 m3u8 直链、网页播放页自动抓取、m3u/m3u8 订阅批量导入
- **🖥️ Web 管理后台** — 访问 `http://你的IP:1905/admin`，可视化管理直播源、分组、台标、EPG 等
...
1. 进后台「**系统配置**」，取消勾选「**启用 H.265 原画**」，点保存——保存即清掉旧的流地址缓存、**即时生效**，无需重启；
2. 在播放端（飞牛影视等）重新打开频道（必要时重新订阅一次）即可。

若仍未生效，重启一次容器即可；用环境变量 `menableH265=false` 部署的，则**必须重启容器**才生效。关闭后回落到兼容性最好的 H.264，清晰度基本无感。若个别频道仍只有声音，再加 `menableHDR=false`（少数频道为 HDR/Vivid 导致）。
...
<details>
<summary><b>📜 查看历史版本更新日志（v3.6.0 → v1.2.4）</b></summary>
...
- 🆕 **支持导入本地 .m3u/.txt 文件批量加源**：「添加源 → 订阅模式」新增「本地文件 / 粘贴」，可选本地 m3u/m3u8/txt 文件、或直接粘贴内容批量导入，自动识别格式与 GBK/UTF-8 编码，全程不发起网络请求（issue #43）
