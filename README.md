...
### 未发布（多套 m3u 配置档 / 大分组，待 bump-version.js 定版）

本次为「多套配置档」大版本：一套服务可维护多套个性化播放列表，给家里不同电视（大厅老人 / 卧室年轻人 / 茶室）各用各的。

- 🆕 **多套配置档（大分组）**：「我的频道」里可新建多套配置档，每套独立保存隐藏 / 分组 / 排序 / 重命名等定制，通过 `/m3u?profile=<短名>`（如 `/m3u?profile=woshi`）访问，互不影响——不同电视填不同地址即可各看各的。底层频道全集 / EPG / 回看多档共享，新增一台电视不增加任何抓取成本
- 🆕 **后台集中管理多档**：「我的频道」首屏只留「当前档：xx」+「✏️ 编辑」一个入口（不喧宾夺主），点开弹窗集中管理——切换 / 新建 / 改名 / 删除，每档显示各自的访问地址可一键复制。新建会**复制当前档**（填中文显示名 + 选填英文地址短名，留空自动生成；档名 ≤20 字符），适合「从大厅档派生卧室档再改两个台」；切换后**记住所选档**（刷新不再跳回默认档）
- 🔧 **完全向后兼容**：默认档仍是原 `my-playlist-config.json`、地址仍是裸 `/m3u`，**老用户与已配好的电视零感知，m3u/txt 地址无需改动、无数据迁移**；新档存为 `my-playlist-config.<短名>.json` + 档清单 `my-playlist-profiles.json`，随 `mdataDir` 数据卷持久化；短名限 `[a-z0-9_-]`、非法/缺省回退默认档。⚠️ 唯一交互变化：「我的频道」的 **🔄 重置按钮已从标题行移入「✏️ 编辑」弹窗**（在当前档那一行，且只重置当前档）
- 🐛 **修复「恢复所有频道」误报**：之前点该按钮会弹「恢复失败: channelIds is not defined」（实为变量名笔误，恢复其实已成功），现正确提示「已恢复 N 个频道」
...
家里多台电视各有偏好（大厅给老人、卧室给年轻人、茶室……），可以用「配置档」给每台电视维护一套独立的频道编排，互不影响。

**怎么用：** 管理后台 →「📺 我的频道」标题行右侧：

- **配置档下拉**：切换当前编辑 / 查看的档（默认档 = 原来的「我的频道」，地址不变）
- **➕ 新建**：复制当前档为新的一档（弹窗填显示名如「卧室」，可选填英文地址短名如 `woshi`，留空自动生成）
- **✏️ 改名 / 🗑️ 删除**：管理非默认档（默认档不可删 / 改名）

**每台电视填各自的地址即可**（每档底部会显示，可直接复制）：
