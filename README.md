...
- 🆕 **新建即复制 + 一键切换**：后台「我的频道」标题行新增配置档下拉与「➕ 新建 / ✏️ 改名 / 🗑️ 删除」；新建会**复制当前档**（弹窗填中文显示名 + 选填英文地址短名，留空自动生成），适合「从大厅档派生卧室档再改两个台」；切换下拉即整套切到该档编辑，每档底部显示自己的访问地址可直接复制
- 🔧 **完全向后兼容**：默认档仍是原 `my-playlist-config.json`、地址仍是裸 `/m3u`，老用户与已配好的电视零感知；新档存为 `my-playlist-config.<短名>.json` + 档清单 `my-playlist-profiles.json`，随 `mdataDir` 数据卷持久化。短名限 `[a-z0-9_-]`、非法/缺省回退默认档
